...
st.title("🛡️ ID-CheckPro: Forensic & Biometric Analyzer")
st.write("Investigation Division - Airport Security Project")

mode = st.sidebar.selectbox("เลือกโหมดการทำงาน", ["Live Camera", "Upload Document"])
...
input_image = None
if mode == "Live Camera":
    img_file = st.camera_input("สแกนพาสปอร์ตหรือใบหน้า")
    if img_file: input_image = Image.open(img_file)
else:
    uploaded_file = st.file_uploader("อัปโหลดภาพพาสปอร์ต", type=['jpg', 'jpeg', 'png'])
...
if input_image:
    ...
    with col1:
        if st.button("🔍 Run Forensic CMYK"):
    ...
    with col2:
        if st.button("👤 Biometric Face Match"):
